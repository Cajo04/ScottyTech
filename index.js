    ...
    sock.ev.on('messages.upsert', async (update) => {
        try {
            ...
            if (chatId === 'status@broadcast') {
                try {
                    const { runAutoViewStatus } = require('./commands/autoviewstatus');
                    await runAutoViewStatus(sock, mek);
                } catch {}
                return;
            }
        ...
        try {
            ...
            for (const update of updates) {
                try {
                    ...
                    // Resolve a display name for whoever deleted the message —
                    // pushName from the original message, falling back to the
                    // saved contact name, then just the phone number.
                    const senderNum = storedSender.split('@')[0];
                    let deleterName = original.pushName || '';
                    if (!deleterName) {
                        try {
                            const storeLib = require('./lib/store');
                            deleterName = storeLib.contacts?.[storedSender]?.name
                                       || storeLib.contacts?.[storedSender]?.notify || '';
                        } catch {}
                    }
                    const deleterLabel = deleterName
                        ? `${deleterName} (@${senderNum})`
                        : `@${senderNum}`;

                    const reportText =
                        `🗑️ *Deleted Message*\n` +
                        `👤 *By:* ${deleterLabel}\n\n` +
                        `${storedBody || '_[media — sent below]_'}`;
